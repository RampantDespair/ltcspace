...
export class PoolRankingComponent implements OnInit {
  ...
  ngOnInit(): void {
    if (this.widget) {
      ...
      this.seoService.setDescription($localize`:@@meta.description.bitcoin.graphs.pool-ranking:See the top Litecoin mining pools ranked by number of blocks mined, over your desired timeframe.`);
    ...
    this.miningStatsObservable$ = merge(
      ...
      .pipe(
        map(data => {
          data['minersLuck'] = (100 * (data.blockCount / 4032)).toFixed(2); // luck 1w (2.5-minute blocks, 4032 blocks per week)
          return data;
        }),
        tap(data => {
