  ...
  ngDoCheck(): void {
    ...
    if (url[0].path === 'faq' ) {
      ...
      if (this.stateService.network === 'liquid' || this.stateService.network === 'liquidtestnet' ) {
        ...
        this.seoService.setDescription($localize`:@@meta.description.docs.rest-bitcoin:Documentation for the Litecoin Space REST API service: get info on addresses, transactions, blocks, fees, mining, the Lightning network, and more.`);
      }
      ...
      if( this.stateService.network === 'liquid' || this.stateService.network === 'liquidtestnet' ) {
        ...
        this.seoService.setDescription($localize`:@@meta.description.docs.websocket-bitcoin:Documentation for the Litecoin Space WebSocket API service: get real-time info on blocks, mempools, transactions, addresses, and more.`);
      }
    } else {
      this.activeTab = 3;
