...
export class NodesPerISPChartComponent implements OnInit {
  ...
  ngOnInit(): void {
    if (!this.widget) {
      ...
      this.seoService.setDescription($localize`:@@meta.description.lightning.nodes-per-isp:Browse the top 100 ISPs hosting Lightning nodes along with stats like total number of nodes per ISP, aggregate LTC capacity per ISP, and more`);
    }
